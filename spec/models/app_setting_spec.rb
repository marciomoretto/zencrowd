...
RSpec.describe AppSetting, type: :model do
  ...
  describe '.min_payment_reais' do
    it 'returns default when record does not exist' do
      expect(described_class.min_payment_reais).to eq(0)
    end

    it 'returns stored value when record exists' do
      described_class.create!(key: described_class::KEY_MIN_PAYMENT_REAIS, value: '25')

      expect(described_class.min_payment_reais).to eq(25)
    end
  end
